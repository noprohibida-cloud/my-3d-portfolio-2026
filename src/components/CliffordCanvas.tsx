...
// ─────────────────────────────────────────────────────────────────────────────
//  CliffordCanvas — canvas position:absolute dans le hero
//  Technique accumulation "flame" : pas de clear → filaments lumineux
//  Morphing lent entre 8 presets Clifford (~12s/transition)
// ─────────────────────────────────────────────────────────────────────────────
...
varying vec2 vUv;
void main() {
  vec2 sampUv = 0.5 + (vUv - 0.5) * 0.9998;
  gl_FragColor = vec4(texture2D(uTex, sampUv).rgb * 0.9985, 1.0);
}`;
...
void main() {
  float raw = texture2D(uTex, vUv).r;

  vec3 col0 = vec3(0.027, 0.031, 0.051);  // #07080d
  vec3 col1 = vec3(0.239, 0.290, 0.541);  // indigo
  vec3 col2 = vec3(0.545, 0.624, 0.878);  // bleu clair
  vec3 col3 = vec3(1.000, 1.000, 1.000);  // blanc
  vec3 col4 = vec3(0.941, 0.769, 0.153);  // #F0C427

  vec3 col;
  float t;
  if (raw < 0.04) {
    col = mix(col0, col1, smoothstep(0.0, 1.0, raw / 0.04));
  } else if (raw < 0.12) {
    col = mix(col1, col2, smoothstep(0.0, 1.0, (raw - 0.04) / 0.08));
  } else if (raw < 0.35) {
    col = mix(col2, col3, smoothstep(0.0, 1.0, (raw - 0.12) / 0.23));
  } else {
    col = mix(col3, col4, smoothstep(0.0, 1.0, clamp((raw - 0.35) / 0.30, 0.0, 1.0)));
  }

  col += (hash(vUv + fract(uTime * 0.0071)) - 0.5) * 0.018 * max(raw * 2.0, 0.25);
...
function makeRT(gl: WebGLRenderingContext, w: number, h: number) {
  ...
  for (let i = 0; i < w * h; i++) { init[i*4]=7; init[i*4+1]=8; init[i*4+2]=13; init[i*4+3]=255; }
...
function fillBatch(pos: Float32Array, state: {x:number;y:number}, a:number, b:number, c:number, d:number) {
  let {x, y} = state;
...
export default function CliffordCanvas() {
  ...
  useEffect(() => {
    ...
    // Dimensions depuis le parent (la section hero)
    ...
    const pAccum   = mkProg(gl, ACCUM_VERT,  ACCUM_FRAG);
    const pDecay   = mkProg(gl, QUAD_VERT,   DECAY_FRAG);
    const pDisplay = mkProg(gl, QUAD_VERT,   DISPLAY_FRAG);
    ...
    const tick = () => {
      ...
      // Injection Clifford dans rtA (PAS de clear)
  ...
  return (
    <canvas
      ...
      style={{
        ...
        height:        "100%",
        pointerEvents: "none",
        zIndex:        0,        // derrière le texte z-[2]
        display:       "block",
      }}
    />
  );
