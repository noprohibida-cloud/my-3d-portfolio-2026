...
const ProjectsSection: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    const tryInit = (): (() => void) => {
      ...
      // ── Affiche/cache le canvas selon que #projects est dans le viewport ──
      triggers.push(
        ScrollTrigger.create({
          trigger:     sectionRef.current!,
          start:       "top bottom",
          end:         "bottom top",
          onEnter:     () => { if (canvas) canvas.style.opacity = "1"; },
          onLeave:     () => { if (canvas) canvas.style.opacity = "0"; },
          onEnterBack: () => { if (canvas) canvas.style.opacity = "1"; },
          onLeaveBack: () => { if (canvas) canvas.style.opacity = "0"; },
        })
      );
  ...
  return (
    ...
    >
      {/* Canvas WebGL — fixed, caché par défaut, visible uniquement dans #projects */}
      <canvas
        ref={canvasRef}
        style={{
          position:      "fixed",
